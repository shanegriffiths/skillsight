...
- [x] **Project-level `.agents/skills` hub scanning.** Universal skill-runtimes (Codex, Gemini, OpenCode) now scan a project's shared `.agents/skills` hub directly (`scanProjectHub` in `skillscan.ts`), the folder-scoped mirror of the global hub. **usedBy semantics:** a bare project-hub skill (no per-runtime symlink dir) reaches every *detected* universal runtime — each adapter contributes itself via the owner fallback and the resolver unions them by `contentId`; already-symlinked skills dedupe to the same record so nothing double-lists. Cursor stays MCP-only (no skills concept). Fixtures in `test/adapters-dir.test.ts` + `test/engine.test.ts`. **Done 2026-07-10.**
